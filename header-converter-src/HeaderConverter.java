...
public class HeaderConverter {
    ...
    private static void processInjectorClass(String inputPath, String outputPath) throws IOException {
        ...
        String varName = outputPath.contains("fabric_injector") ? "fabric_injector_class_data" : "injector_class_data";
        String guardName = outputPath.contains("fabric_injector") ? "CLASSES_FABRIC_INJECTOR_H_" : "CLASSES_INJECTOR_H_";

        StringBuilder resultFile = new StringBuilder();
        resultFile.append("#ifndef ").append(guardName).append("\n#define ").append(guardName)
            .append("\n\n#include \"../jvm/jni.h\"\n\nconst jbyte ").append(varName).append("[] = ")
            .append(cppDataString)
            .append(";\n\n#endif  //").append(guardName);
    ...
    private static void processInputJar(String inputPath, String outputPath) throws IOException {
            ...
            for (ZipEntry entry : jarFile.stream().collect(Collectors.toList())) {
                ...
                if (entryName.equals("fabric.mod.json")) {
                    try (InputStream stream = jarFile.getInputStream(entry)) {
                        fabricModJsonBytes = readStreamFully(stream);
                        System.out.println("Detected fabric.mod.json ("
                                + fabricModJsonBytes.length + " bytes) — Fabric mod");
                    }
                    continue;
                }
        ...
        boolean isFabricMod = (fabricModJsonBytes != null);
        System.out.println("Processing " + classes.size() + " classes");
        System.out.println("Mod type: " + (isFabricMod ? "Fabric" : "Forge"));
        ...
        resultFile.append("#include \"../jvm/jni.h\"\n\n");

        if (isFabricMod) {
            resultFile.append("#define IS_FABRIC_MOD 1\n");
            resultFile.append("#define IS_FORGE_MOD  0\n\n");
            resultFile.append("const jbyte fabric_mod_json_data[] = ")
                    .append(convertByteArrayToCppString(fabricModJsonBytes))
                    .append(";\n");
            resultFile.append("const jint fabric_mod_json_size = ")
                    .append(fabricModJsonBytes.length)
                    .append(";\n\n");
        } else {
            resultFile.append("#define IS_FABRIC_MOD 0\n");
            resultFile.append("#define IS_FORGE_MOD  1\n\n");
            resultFile.append("const jbyte fabric_mod_json_data[] = {0x00};\n");
            resultFile.append("const jint fabric_mod_json_size = 0;\n\n");
        }

        for (int i = 0; i < classDataStrings.size(); i++) {
            resultFile.append("const jbyte class_data_").append(i)
                    .append("[] = ").append(classDataStrings.get(i)).append(";\n");
        }
