...
static jclass DefineOrGetInjector(JNIEnv* jni_env) {
  ...
  if (existing) {
    ShowMessage(L"FabricInjector already in JVM, reusing it");
    return existing;
  }
...
#if IS_FABRIC_MOD
/** Wraps fabric_mod_json_data into a jbyteArray to pass to FabricInjector.inject(). */
